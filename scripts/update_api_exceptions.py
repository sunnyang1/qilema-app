...
FILES_TO_UPDATE = [
    ...
    },
    {
        ...
        "replacements": [
            (
                'raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="SOS记录不存在")',
                'raise NotFoundException("SOS记录不存在")',
            ),
            (
                'raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="只能取消待处理的SOS求助")',

...
class AliyunVoiceAdapter(PhoneNotificationSimulator):
    ...
    def __init__(
        ...
        """
        super().__init__(
            enabled, success_rate, delay_ms, max_retries, retry_interval_ms, tts_voice
        )

        # 检查是否使用真实服务
        self.use_real_service = (
            os.getenv("PHONE_USE_REAL_SERVICE", "false").lower() == "true"
        )
        ...
        self.called_show_number = called_show_number or os.getenv(
            "ALIYUN_VOICE_SHOW_NUMBER"
        )
    ...
    def _init_aliyun_client(self):
        ...
        try:
            ...
            logger.warning(
                "阿里云语音SDK未安装，将使用模拟器模式。运行: pip install alibabacloud_dyvmsapi20170525"
            )
    ...
    def _send(
        ...
        **kwargs,
        ...
        if call_type == "verify":
            ...
            return self._send_tts_call(
                phone_number, content, template_code, template_params
            )
    ...
    def _send_tts_call(
        ...
        try:
            if template_code:
                # 使用模板发送
                single_call_by_tts_request = (
                    self._dyvmsapi_models.SingleCallByTtsRequest(
                        called_number=phone_number,
                        called_show_number=self.called_show_number,
                        tts_code=template_code,
                        tts_param=(
                            json.dumps(template_params) if template_params else None
                        ),
    ...
    def _send_verify_call(self, phone_number: str, code: str) -> Dict[str, Any]:
        ...
        """
        try:
            single_call_by_voice_request = (
                self._dyvmsapi_models.SingleCallByVoiceRequest(
                    called_number=phone_number,
                    called_show_number=self.called_show_number,
                    voice_code=code,
                )
    ...
    def _handle_voice_response(
        self, response, phone_number: str, call_type: str, content: str
    ) -> Dict[str, Any]:
        """处理语音服务响应
        ...
        if response_body.code == "OK":
            ...
            logger.error(
                f"语音电话发送失败 - 手机:{phone_number}, 错误:{error_code} - {error_message}"
            )
    ...
    def get_call_status(self, call_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            query_call_detail_by_call_id_request = (
                self._dyvmsapi_models.QueryCallDetailByCallIdRequest(
                    call_id=call_id,
            ...
            response = self._client.query_call_detail_by_call_id(
                query_call_detail_by_call_id_request
            )
    ...
    def call(self, phone_number: str, content: str, **kwargs) -> Dict[str, Any]:
        ...
        """
        return self.send(
            phone_number=phone_number, content=content, call_type="tts", **kwargs
        )
    ...
    def send_verify_code(self, phone_number: str, code: str) -> Dict[str, Any]:
        ...
        Returns:
            dict: 发送结果
        """
        return self.send(phone_number=phone_number, content=code, call_type="verify")
...
def create_aliyun_voice_adapter(

...
def get_adapter_config(channel: str) -> Dict[str, Any]:
    ...
    if channel == "sms":
        ...
        config.update({"push_token": os.getenv("PUSH_TOKEN")})
    elif channel == "phone":
        config.update({"tts_voice": os.getenv("TTS_VOICE")})
    elif channel == "email":
        config.update({"smtp_server": os.getenv("SMTP_SERVER")})

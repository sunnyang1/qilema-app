    ...
    def get_comprehensive_report(
        ...
        return {
            ...
            "user_profile": {
                ...
                "record_updated_at": (
                    health_record.updated_at.isoformat() if health_record else None
                ),
            },
    ...
    def get_daily_summary(
        ...
        if bp_data:
            summary["metrics"]["blood_pressure"] = {
                "systolic_avg": round(
                    sum(d.get("systolic", 0) for d in bp_data) / len(bp_data), 1
                ),
                "diastolic_avg": round(
                    sum(d.get("diastolic", 0) for d in bp_data) / len(bp_data), 1
                ),
    ...
    def _detect_anomalies(
        ...
        for dp in data_points:
            ...
            if min_val is not None and value < min_val:
                diff_percent = (
                    abs(value - min_val) / min_val * 100 if min_val > 0 else 0
                )
                if diff_percent > 20:
                    severity = "high"

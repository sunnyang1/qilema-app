...
class AlertService(BaseService[Alert]):
    ...
    def create_or_update_setting(
        self, user_id: str, setting_data: AlertSettingCreate
    ) -> AlertSetting:
        """创建或更新预警配置"""
        # 查找现有配置
        setting = (
            self.db.query(AlertSetting).filter(AlertSetting.user_id == user_id).first()
        )
    ...
    def get_setting(self, user_id: str) -> Optional[AlertSetting]:
        """获取预警配置"""
        # 尝试从缓存获取
    ...
    def update_setting(
        self, user_id: str, update_data: AlertSettingUpdate
    ...
    def auto_resolve_by_checkin(self, user_id: str) -> int:
        """签到后自动解除所有活动预警"""
        setting = self.get_setting(user_id)
    ...
    def get_contacts_for_notification(self, user_id: str) -> List[EmergencyContact]:
        """获取用于通知的紧急联系人"""
        setting = self.get_setting(user_id)
        if setting and not setting.emergency_contact_notify:
            return []

        return self.get_user_emergency_contacts(user_id)

    def check_all_users_and_create_alerts(self) -> List[Alert]:
        """检查所有用户并创建预警"""
        created_alerts = []

        # 获取所有启用了预警的用户配置
        settings = (
            self.db.query(AlertSetting)
            .filter(
        ...
        for setting in settings:
            # 检查签到状态
            status = self.check_user_checkin_status(setting.user_id)
            if status and status["trigger_alert"]:
                # 计算严重程度
                severity = self._calculate_severity(int(status["missed_hours"]))
    ...
    def get_pending_alerts(self) -> List[Alert]:
        ...
        return (
            ...
            .order_by(Alert.trigger_time)
            .all()
        )

    def check_missed_checkin(self, user_id: str) -> Optional[Alert]:
        """检查是否未签到"""
        setting = self.get_setting(user_id)
    ...
    def check_abnormal_data(self, user_id: str, health_data: dict) -> Optional[Alert]:
        """检查生理数据异常"""
        setting = self.get_setting(user_id)
    ...
    def _check_heart_rate_abnormal(
        self, health_data: dict, setting: AlertSetting, abnormal_info: dict
    ):
        """检查心率异常"""
    ...
    def get_user_emergency_contacts(self, user_id: str) -> List[EmergencyContact]:
        """获取用户紧急联系人"""
        return (
    ...
    def _calculate_severity(self, missed_hours: int) -> str:
        """计算严重程度"""
        if missed_hours >= 96:
            return "critical"
    ...
    def check_user_checkin_status(self, user_id: str) -> Optional[dict]:
        """检查用户签到状态"""
        setting = self.get_setting(user_id)
        if (
            not setting

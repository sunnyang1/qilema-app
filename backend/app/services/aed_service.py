...
from app.models.emergency_resource_model import EmergencyResource, AEDStatus
...
class AEDService(BaseService[EmergencyResource]):
    ...
    @classmethod
    def get_aed_statistics(cls, db: Session, city: Optional[str] = None) -> Dict[str, Any]:
        """
        获取AED统计信息
        ...
        # 按城市统计（前10）
        city_stats = db.query(
            EmergencyResource.city,
            func.count(EmergencyResource.id).label("count")
        ).filter(
            EmergencyResource.resource_type == "aed"
        ).group_by(
            EmergencyResource.city
        ).order_by(func.count(EmergencyResource.id).desc()).limit(10).all()
        ...
        return {
            ...
            "active_rate": round(
                (status_counts.get("active", 0) + status_counts.get("unknown", 0)) / total * 100, 2
            ) if total > 0 else 0
        }
    ...
    def get_expiring_aeds(
        ...
        aeds = db.query(EmergencyResource).filter(
            EmergencyResource.resource_type == "aed",
            EmergencyResource.is_active == 1,
            or_(
    ...
    @classmethod
    def _create_aed_from_dict(cls, db: Session, data: Dict[str, Any]) -> EmergencyResource:
        """从字典创建AED设备"""
        aed = EmergencyResource(
            ...
            aed_location_desc=data.get("aed_location_desc") or data.get("location_desc"),
            aed_access_instructions=data.get("aed_access_instructions") or data.get("access_instructions"),
            aed_manager_name=data.get("aed_manager_name") or data.get("manager_name"),
            aed_manager_phone=data.get("aed_manager_phone") or data.get("manager_phone"),
                ...
                if key in data and data[key]:
                    try:
                        if isinstance(data[key], str):
                            date_val = datetime.fromisoformat(data[key].replace("Z", "+00:00"))
                        else:
                            date_val = data[key]
    ...
    @staticmethod
    def _calculate_search_boundaries(latitude: float, longitude: float, radius: float) -> Dict[str, float]:
        """计算搜索边界的经纬度"""

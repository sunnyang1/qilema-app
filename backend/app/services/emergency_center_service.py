...
class EmergencyCenterService:
    ...
    def call_120(self, request: Call120Request) -> Call120Response:
        ...
        return Call120Response(
            ...
            emergency_center_name=(
                emergency_center.center_name if emergency_center else None
            ),
            emergency_phone=(
                emergency_center.emergency_phone if emergency_center else "120"
            ),
    ...
    def _send_health_summary_to_120(self, call: EmergencyCall, user_id: str) -> bool:
        ...
        try:
            ...
            call.health_summary_content = json.dumps(
                health_summary.model_dump(mode="json"), ensure_ascii=False
            )
    ...
    def generate_health_summary(self, user_id: str) -> HealthSummary:
        ...
        summary = HealthSummary(
            ...
            chronic_diseases=(
                json.loads(health_record.chronic_diseases_json)
                if health_record and health_record.chronic_diseases_json
                else None
            ),
            allergies=(
                json.loads(health_record.allergies_json)
                if health_record and health_record.allergies_json
                else None
            ),
            current_medications=(
                json.loads(health_record.current_medications_json)
                if health_record and health_record.current_medications_json
                else None
            ),
            latest_heart_rate=(
                latest_device_data.heart_rate if latest_device_data else None
            ),
            latest_blood_pressure=(
                f"{latest_device_data.systolic_pressure}/{latest_device_data.diastolic_pressure}"
                if latest_device_data and latest_device_data.systolic_pressure
                else None
            ),
            latest_blood_oxygen=(
                latest_device_data.blood_oxygen if latest_device_data else None
            ),
    ...
    def update_rescue_record(
        ...
        if update_data.arrival_time and record.alarm_time:
            record.response_time_minutes = int(
                (update_data.arrival_time - record.alarm_time).total_seconds() / 60
            )

        # 计算总时长
        if update_data.completion_time and record.incident_time:
            record.overall_duration_minutes = int(
                (update_data.completion_time - record.incident_time).total_seconds()
                / 60
            )
    ...
    def _call_emergency_center_api(
        self, emergency_center: EmergencyCenter, method: str, data: dict
    ):
        """
        调用急救中心API

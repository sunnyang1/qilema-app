...
class Anomaly(Base, BaseModelMixin):
    ...
    # 异常信息
    anomaly_type = Column(
        SQLEnum(AnomalyTypeEnum), nullable=False, index=True, comment="异常类型"
    )
    severity = Column(
        SQLEnum(SeverityLevel), nullable=False, index=True, comment="严重程度"
    )
    status = Column(
        SQLEnum(AnomalyStatus),
...
class HealthTrend(Base, BaseModelMixin):
    ...
    metric_type = Column(
        ...
        nullable=False,
        index=True,
        comment="指标类型(heart_rate/blood_pressure/etc)",
    )
    period_type = Column(
        String(20), nullable=False, comment="周期类型(daily/weekly/monthly)"
    )

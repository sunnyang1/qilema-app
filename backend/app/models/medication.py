...
class MedicationReminderItem(Base, BaseModelMixin):
    ...
    medication_type = Column(
        Enum(MedicationType), default=MedicationType.PRESCRIPTION, comment="药品类型"
    )
    ...
    schedules = relationship(
        ...
        back_populates="medication_item",
...
class MedicationReminderSchedule(Base, BaseModelMixin):
    ...
    medication_item_id = Column(
        Integer,
        ForeignKey("medication_reminder_items.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
        comment="药品ID",
    )
...
class MedicationReminderNotification(Base, BaseModelMixin):
    ...
    schedule_id = Column(
        Integer,
        ForeignKey("medication_reminder_schedules.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
        comment="用药计划ID",
    )
    ...
    # 状态
    status = Column(
        Enum(ReminderStatus), default=ReminderStatus.PENDING, comment="提醒状态"
    )
    ...
    log_id = Column(
        Integer,
        ForeignKey("medication_reminder_logs.id", ondelete="SET NULL"),
        comment="对应的服药记录ID",
    )
...
class MedicationReminderLog(Base, BaseModelMixin):
    ...
    schedule_id = Column(
        Integer,
        ForeignKey("medication_reminder_schedules.id", ondelete="SET NULL"),
        comment="用药计划ID",
    )
    reminder_id = Column(
        Integer,
        ForeignKey("medication_reminder_notifications.id", ondelete="SET NULL"),
        comment="对应的提醒ID",
    )

...
ModernUser 等为演示用，勿与生产 User 混用；归属见 docs/PHASE2_DOMAIN_BOUNDARIES.md（R-W4）。
"""
...
class ModernUser(Base, BaseModelMixin):
    ...
    updated_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), nullable=True, onupdate=datetime.utcnow, comment="更新时间"
    )
...
class ModernPost(Base, BaseModelMixin):
    ...
    post_id: Mapped[str] = mapped_column(String(36), primary_key=True, comment="帖子ID")

    title: Mapped[str] = mapped_column(String(200), nullable=False, comment="标题")

    content: Mapped[Optional[str]] = mapped_column(
        String(5000), nullable=True, comment="内容"
    )

    # 外键 - SQLAlchemy 2.x 使用 mapped_column(ForeignKey(...))
    author_id: Mapped[str] = mapped_column(
        ForeignKey("modern_users.user_id"), nullable=False, comment="作者ID"
    )
...
class ModernUserProfile(Base, BaseModelMixin):
    ...
    profile_id: Mapped[str] = mapped_column(
        String(36), primary_key=True, comment="资料ID"
    )

...
""")
        ...
        # 设置环境变量
        monkeypatch.setenv('DATABASE_URL', 'postgresql://override:override@localhost:5432/override')
...
class TestDockerComposeFiles:
    ...
    def test_dev_compose_config(self):
        ...
        assert compose_data['services']['backend']['environment']['ENVIRONMENT'] == 'development'
    ...
    def test_prod_compose_config(self):
        ...
        # 验证生产环境特定配置
        assert 'backend' in compose_data['services']
        assert compose_data['services']['backend']['environment']['ENVIRONMENT'] == 'production'
        assert compose_data['services']['backend']['deploy']['resources']['limits']['memory'] == '2G'

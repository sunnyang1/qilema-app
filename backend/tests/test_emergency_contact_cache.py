...
class TestEmergencyContactCacheInvalidation:
    ...
    def test_create_emergency_contact_invalidates_list_cache(self, mock_invalidate, service, mock_db, sample_user_id):
        """测试创建联系人时失效列表缓存"""
        # Given
    ...
    @patch('app.services.emergency_contact_service.invalidate_cache')
    def test_update_emergency_contact_invalidates_caches(self, mock_invalidate, service, mock_db, sample_user_id):
        """测试更新联系人时失效单个和列表缓存"""
        # Given
        contact_id = 1
        ...
        with patch.object(service, 'get_emergency_contact', return_value=sample_contact):
            ...
            # When
            service.update_emergency_contact(mock_db, contact_id, update_data, sample_user_id)

        # Then
        calls = mock_invalidate.call_args_list
        assert any(f"emergency:contact:{contact_id}:{sample_user_id}" in str(call) for call in calls)
        assert any(f"emergency:contacts:{sample_user_id}" in str(call) for call in calls)

    @patch('app.services.emergency_contact_service.invalidate_cache')
    def test_delete_emergency_contact_invalidates_caches(self, mock_invalidate, service, mock_db, sample_user_id):
        """测试删除联系人时失效单个和列表缓存"""
        # Given
        contact_id = 1
        sample_contact = Mock(spec=EmergencyContact)
    ...
    @patch('app.services.emergency_contact_service.invalidate_cache')
    def test_set_primary_contact_invalidates_list_cache(self, mock_invalidate, service, mock_db, sample_user_id):
        """测试设置主要联系人时失效列表缓存"""
        # Given
        contact_id = 1
        sample_contact = Mock(spec=EmergencyContact)

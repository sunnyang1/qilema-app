...
class TestAEDService:
    ...
    def test_get_nearby_aeds(self, db: Session):
        ...
        # 搜索附近的AED
        aeds = AEDService(db).get_nearby_aeds(
            latitude=31.2304, longitude=121.4737, radius=1000, limit=10  # 1公里
        )
    ...
    def test_get_nearby_aeds_only_active(self, db: Session):
        ...
        aeds = AEDService(db).get_nearby_aeds(
            latitude=31.2304, longitude=121.4737, radius=1000, only_active=True
        )

        assert len(aeds) == 1
        assert aeds[0]["resource_name"] == "可用AED"

    def test_get_aed_navigation_url(self, db: Session):
        """测试获取导航链接"""
        nav_urls = AEDService(db).get_aed_navigation_url(
            from_lat=31.2304,
            from_lon=121.4737,
            to_lat=31.2314,
            to_lon=121.4747,
    ...
    def test_import_aeds_batch(self, db: Session):
        ...
        result = AEDService(db).import_aeds_batch(aed_data)
    ...
    def test_get_aed_statistics(self, db: Session):
        ...
        stats = AEDService(db).get_aed_statistics()
    ...
    def test_get_aed_statistics_by_city(self, db: Session):
        ...
        sh_stats = AEDService(db).get_aed_statistics(city="上海市")
        assert sh_stats["total"] == 2

        bj_stats = AEDService(db).get_aed_statistics(city="北京市")
    ...
    def test_update_aed_status(self, db: Session):
        ...
        updated = AEDService(db).update_aed_status(
            aed.id,
    ...
    def test_get_expiring_aeds(self, db: Session):
        ...
        expiring = AEDService(db).get_expiring_aeds(days=30)
    ...
    def test_calculate_distance(self, db: Session):
        """测试距离计算"""
        # 上海人民广场到外滩的距离约1.5公里
        distance = AEDService(db)._calculate_distance(
    ...
    def test_format_distance(self, db: Session):
        """测试距离格式化"""
        assert AEDService(db)._format_distance(500) == "500米"
        assert AEDService(db)._format_distance(1500) == "1.5公里"
        assert AEDService(db)._format_distance(10000) == "10.0公里"

    def test_format_duration(self, db: Session):
        """测试时长格式化"""
        assert AEDService(db)._format_duration(30) == "1分钟"
        assert AEDService(db)._format_duration(300) == "5分钟"
        assert AEDService(db)._format_duration(3600) == "1小时"
        assert AEDService(db)._format_duration(3660) == "1小时1分钟"

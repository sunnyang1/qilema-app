...
class TestAEDService:
    ...
    def test_get_nearby_aeds(self, db: Session):
        """测试获取附近AED设备"""
        # 创建多个AED设备
        self._create_test_aed(db, resource_name="AED-1", latitude=31.2304, longitude=121.4737)
        self._create_test_aed(db, resource_name="AED-2", latitude=31.2314, longitude=121.4747)
        self._create_test_aed(db, resource_name="AED-3", latitude=31.2324, longitude=121.4757)
        # 远离的AED
        self._create_test_aed(db, resource_name="AED-远", latitude=31.3000, longitude=121.5500)
    ...
    def test_get_nearby_aeds_only_active(self, db: Session):
        """测试只返回可用状态的AED"""
        self._create_test_aed(db, resource_name="可用AED", aed_status=AEDStatus.ACTIVE.value)
        self._create_test_aed(db, resource_name="维护AED", aed_status=AEDStatus.MAINTENANCE.value)
        self._create_test_aed(db, resource_name="不可用AED", aed_status=AEDStatus.INACTIVE.value)

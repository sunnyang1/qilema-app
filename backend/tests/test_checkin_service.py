...
class TestCheckInService:
    ...
    def test_get_checkin_status_specific_date(self, db, test_user):
        ...
        # 查询昨天状态
        status = CheckInService.get_checkin_status(db, test_user.user_id, date.today() - timedelta(days=1))
        
        assert status.is_checked_in is True
        assert status.checkin_time is not None

    def test_get_emergency_contacts_for_notification(self, db, test_user, test_contacts):
        """测试获取紧急联系人(用于通知)"""
        contacts = CheckInService.get_emergency_contacts_for_notification(db, test_user.user_id)
    ...
    def test_calculate_longest_streak(self, db, test_user):
        ...
        # 计算最长连续签到
        longest = CheckInService._calculate_longest_streak(db, test_user.user_id, days=10)

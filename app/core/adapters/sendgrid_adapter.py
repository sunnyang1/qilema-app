...
class SendGridAdapter(EmailNotificationSimulator):
    ...
    def __init__(
        ...
        Args:
            ...
            from_name: 发件人名称
        """
        super().__init__(enabled, success_rate, delay_ms, max_retries, retry_interval_ms, smtp_server)
        
        # 检查是否使用真实服务
        self.use_real_service = os.getenv("EMAIL_USE_REAL_SERVICE", "false").lower() == "true"
        
        # SendGrid配置
        self.api_key = api_key or os.getenv("SENDGRID_API_KEY")
        self.from_email = from_email or os.getenv("SENDGRID_FROM_EMAIL", "noreply@qilema.com")
    ...
    def _send(
        ...
        **kwargs
        ...
        if not self.use_real_service:
            return super()._send(to_email, subject, content, html_content, attachments, **kwargs)
        
        return self._send_real(
            to_email, subject, content, html_content, 
            attachments, template_id, template_data
        )
    ...
    def _send_real(
        ...
        try:
            ...
            if response.status_code in [200, 202]:
                ...
                logger.info(f"SendGrid邮件发送成功 - 收件人:{to_email}, 主题:{subject}, 消息ID:{message_id}")
    ...
    def verify_webhook_signature(self, signature: str, timestamp: str, payload: str) -> bool:

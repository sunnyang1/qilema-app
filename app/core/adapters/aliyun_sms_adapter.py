...
class AliyunSMSAdapter(SMSNotificationSimulator):
    ...
    def __init__(
        ...
        Args:
            ...
            sign_name: 短信签名名称
        """
        super().__init__(enabled, success_rate, delay_ms, max_retries, retry_interval_ms, phone_number)
        
        # 检查是否使用真实服务
        self.use_real_service = os.getenv("SMS_USE_REAL_SERVICE", "false").lower() == "true"
    ...
    def _init_aliyun_client(self):
        ...
        try:
            ...
            logger.warning("阿里云SDK未安装，将使用模拟器模式。运行: pip install alibabacloud_dysmsapi20170525")
    ...
    def _send(
        ...
        **kwargs
        ...
        if not self.use_real_service:
            # 使用模拟器
            return super()._send(phone_number, content, template_code, template_params, **kwargs)
    ...
    def _send_real(
        ...
        try:
            ...
            if response_body.code == "OK":
                ...
                logger.info(f"阿里云短信发送成功 - 手机:{phone_number}, 模板:{template_code}, 业务ID:{message_id}")
                ...
                logger.error(f"阿里云短信发送失败 - 手机:{phone_number}, 错误:{error_code} - {error_message}")
    ...
    def get_send_status(self, message_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            query_request = self._dysmsapi_models.QuerySendDetailsRequest(
                phone_number=self._send_records.get(message_id, {}).get("phone_number", ""),
                biz_id=message_id,
...
def create_aliyun_sms_adapter(config: Optional[Dict[str, Any]] = None) -> AliyunSMSAdapter:
    """创建阿里云短信适配器

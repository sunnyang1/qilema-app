...
from app.core.cache import cache_result, invalidate_cache, get_cached
...
class BaseService(Generic[ModelType]):
    ...
    @classmethod
    def get_by_id(cls, db: Session, id_value: Any, pk_column: str = "id") -> Optional[ModelType]:
        """
        根据ID获取记录
    ...
    @classmethod
    def get_by_field(cls, db: Session, field_name: str, field_value: Any) -> Optional[ModelType]:
        """
        根据字段值获取单条记录
        ...
        **filters
        ...
        for field_name, field_value in filters.items():
            if field_value is not None and hasattr(cls.model_class, field_name):
                query = query.filter(getattr(cls.model_class, field_name) == field_value)

...
from app.schemas.emergency_resource import (
    ...
    ResourceFacilityCreate, ResourceDepartmentCreate,
    ResourceUsageLogCreate, ResourceStatistics, PopularResource
)
...
class EmergencyResourceService:
    ...
    def _get_route_info(
        ...
        return {
            ...
            "steps": [
                ...
                {"instruction": f"前往目的地,距离{distance:.0f}米", "distance": distance}
            ]
        }
    
    # ========== 资源管理 ==========
    
    def create_resource(self, db: Session, resource_data: ResourceCreate) -> EmergencyResource:
        """创建急救资源"""
    ...
    def update_resource(self, db: Session, resource_id: int, update_data: ResourceUpdate) -> Optional[EmergencyResource]:
        """更新急救资源"""
    ...
    def get_resource(self, db: Session, resource_id: int) -> Optional[EmergencyResource]:
        """获取急救资源详情"""
        return db.query(EmergencyResource).filter(EmergencyResource.id == resource_id).first()
    
    def query_resources(self, db: Session, query_params: ResourceQuery) -> List[EmergencyResource]:
        """查询急救资源"""
        query = db.query(EmergencyResource)
        
        if query_params.resource_type:
            query = query.filter(EmergencyResource.resource_type == query_params.resource_type.value)
        ...
        if query_params.has_emergency is not None:
            query = query.filter(EmergencyResource.has_emergency == query_params.has_emergency)
        
        if query_params.hospital_level:
            query = query.filter(EmergencyResource.hospital_level == query_params.hospital_level.value)
        
        if query_params.has_icu is not None:
            query = query.filter(EmergencyResource.has_icu == query_params.has_icu)
        
        return query.order_by(EmergencyResource.created_at.desc()).offset(query_params.offset).limit(query_params.limit).all()
    ...
    def create_facility(self, db: Session, facility_data: ResourceFacilityCreate) -> ResourceFacility:
        """创建资源设施"""
    ...
    def get_resource_facilities(self, db: Session, resource_id: int) -> List[ResourceFacility]:
        """获取资源设施列表"""
        return db.query(ResourceFacility).filter(
            ResourceFacility.resource_id == resource_id
        ).all()
    
    # ========== 资源科室管理 ==========
    
    def create_department(self, db: Session, department_data: ResourceDepartmentCreate) -> ResourceDepartment:
        """创建资源科室"""
    ...
    def get_resource_departments(self, db: Session, resource_id: int) -> List[ResourceDepartment]:
        """获取资源科室列表"""
        return db.query(ResourceDepartment).filter(
            ResourceDepartment.resource_id == resource_id
        ).all()
    ...
    def get_resource_statistics(self, db: Session) -> ResourceStatistics:
        ...
        for resource_type in ResourceType:
            count = db.query(func.count(EmergencyResource.id)).filter(
                EmergencyResource.resource_type == resource_type.value
            ).scalar()
        ...
        for status in ResourceStatus:
            count = db.query(func.count(EmergencyResource.id)).filter(
                EmergencyResource.status == status.value
            ).scalar()
            by_status[status.value] = count
        
        # 按城市分组
        by_city = {}
        cities = db.query(EmergencyResource.city).filter(
            EmergencyResource.city.isnot(None)
        ).distinct().all()
        for city in cities:
            count = db.query(func.count(EmergencyResource.id)).filter(
                EmergencyResource.city == city[0]
            ).scalar()
            by_city[city[0]] = count
        
        # 统计特定资源
        hospitals_with_emergency = db.query(func.count(EmergencyResource.id)).filter(
            EmergencyResource.resource_type == ResourceType.HOSPITAL.value,
            EmergencyResource.has_emergency == True
        ).scalar()
        
        aed_count = db.query(func.count(EmergencyResource.id)).filter(
            EmergencyResource.resource_type == ResourceType.AED.value
        ).scalar()
        
        aed_24h_accessible = db.query(func.count(EmergencyResource.id)).filter(
            EmergencyResource.resource_type == ResourceType.AED.value,
    ...
    def get_popular_resources(self, db: Session, limit: int = 10) -> List[PopularResource]:
        """获取热门资源"""
        # 统计查看次数和导航次数
        view_counts = db.query(
            ...
            ResourceUsageLog.usage_type == "view"
        ).group_by(
            ResourceUsageLog.resource_id
        ).all()
        
        navigate_counts = db.query(
            ResourceUsageLog.resource_id,
            func.count(ResourceUsageLog.id).label('navigate_count')
        ).filter(
            ResourceUsageLog.usage_type == "navigate"
        ).group_by(
            ResourceUsageLog.resource_id
        ).all()
        ...
        for resource_id, view_count in view_counts:
            resource_stats[resource_id] = {"view_count": view_count, "navigate_count": 0}
        ...
        for resource_id, navigate_count in navigate_counts:
            if resource_id in resource_stats:
                ...
                resource_stats[resource_id] = {"view_count": 0, "navigate_count": navigate_count}
        ...
        for resource_id, stats in resource_stats.items():
            ...
            if resource:
                popular_resources.append(PopularResource(
                    id=resource.id,
        ...
        popular_resources.sort(
            key=lambda x: x.view_count + x.navigate_count,
            reverse=True
        )

...
class KnowledgeCategoryService(BaseService[KnowledgeCategory]):
    ...
    @classmethod
    def get_active_categories(cls, db: Session, parent_id: Optional[int] = None) -> List[KnowledgeCategory]:
        """获取启用的分类列表
    ...
    @classmethod
    def update_category(cls, db: Session, category_id: int, data: Dict[str, Any]) -> Optional[KnowledgeCategory]:
        """更新分类
...
class KnowledgeTagService(BaseService[KnowledgeTag]):
    ...
    def get_active_tags(cls, db: Session) -> List[KnowledgeTag]:
        ...
        Returns:
            标签列表
        """
        return db.query(KnowledgeTag).filter(
            KnowledgeTag.is_active == True
        ).order_by(KnowledgeTag.name.asc()).all()
    ...
    @classmethod
    def search_tags(cls, db: Session, keyword: str, limit: int = 10) -> List[KnowledgeTag]:
        """搜索标签
...
class KnowledgeArticleService(BaseService[KnowledgeArticle]):
    ...
    def search_articles(
        ...
        ).order_by(desc(KnowledgeArticle.published_at)).offset(skip).limit(limit).all()
    
    @classmethod
    def get_article_detail(cls, db: Session, article_id: int) -> Optional[KnowledgeArticle]:
        """获取文章详情
...
class KnowledgeBaseService:
    ...
    def get_homepage_data(cls, db: Session) -> Dict[str, Any]:
        ...
        # 获取置顶文章
        top_articles = db.query(KnowledgeArticle).filter(
            and_(
                KnowledgeArticle.is_active == True,
                KnowledgeArticle.status == "published",
                KnowledgeArticle.is_top == True
            )
        ).order_by(desc(KnowledgeArticle.published_at)).limit(5).all()
        
        # 获取最新文章
        latest_articles = cls.article_service.get_published_articles(db, limit=10)
        
        # 获取热门标签
        popular_tags = db.query(KnowledgeTag).filter(
            KnowledgeTag.is_active == True
        ).limit(20).all()

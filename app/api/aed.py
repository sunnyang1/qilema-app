...
@router.get("/{aed_id}", response_model=dict)
def get_aed_detail(
    aed_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_active_user)
...
@router.put("/{aed_id}/status", response_model=dict)
...
@router.get("/statistics/overview", response_model=AEDStatistics)
def get_aed_statistics(
    ...
    return AEDStatistics(**stats)


@router.get("/maintenance/expiring", response_model=dict)
...
@router.get("/map/bounds", response_model=dict)

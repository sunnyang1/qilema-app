...
class TestDeviceService:
    ...
    def test_unbind_device_success(self, db_session, test_user, test_device, device_service):
        """测试成功解绑设备"""
        result = device_service.unbind_device(db_session, test_device.id, test_user.user_id)
    ...
    def test_get_user_devices_with_inactive(self, db_session, test_user, test_device, device_service):
        """测试获取包含已解绑设备的列表"""
        # 解绑一个设备
        device_service.unbind_device(db_session, test_device.id, test_user.user_id)
        
        # 不包含已解绑设备
        active_devices = device_service.get_user_devices(db_session, test_user.user_id, include_inactive=False)
        assert len(active_devices) == 0
        
        # 包含已解绑设备
        all_devices = device_service.get_user_devices(db_session, test_user.user_id, include_inactive=True)
        assert len(all_devices) == 1
    
    def test_get_device_success(self, db_session, test_user, test_device, device_service):
        """测试获取设备详情"""
        device = device_service.get_device(db_session, test_device.id, test_user.user_id)
    ...
    def test_upload_device_data_heart_rate(self, db_session, test_user, test_device, device_service):
        """测试上传心率数据"""
    ...
    def test_upload_device_data_no_data(self, db_session, test_user, test_device, device_service):
        """测试上传无数据(应该失败)"""
        data = DeviceDataUpload(
            device_id=test_device.device_id,
            data_timestamp=datetime.utcnow()
    ...
    def test_upload_device_data_invalid_device(self, db_session, test_user, device_service):
        """测试上传到不存在的设备"""
        data = DeviceDataUpload(
    ...
    def test_get_device_data_by_time_range(self, db_session, test_user, test_device, device_service):
        """测试按时间范围查询设备数据"""
        # 上传多条数据
    ...
    def test_update_device_status(self, db_session, test_user, test_device, device_service):
        """测试更新设备状态"""
        ...
        device = device_service.update_device_status(db_session, test_device.id, test_user.user_id, status_data)
        
        assert device.is_online == True
        assert device.battery_level == 85
    
    def test_check_offline_devices(self, db_session, test_user, test_device, device_service):
        """测试检查离线设备"""
        # 设置设备为在线但很久未同步
        ...
        # 检查离线设备(阈值为60分钟)
        offline_devices = device_service.check_offline_devices(db_session, offline_threshold_minutes=60)
    ...
    def test_update_threshold(self, db_session, test_device, device_service):
        ...
        threshold = device_service.update_threshold(db_session, test_device.id, update_data)
    ...
    def test_detect_high_heart_rate(self, db_session, test_user, test_device, device_service):
        """测试检测心率过高"""
        # 创建阈值配置
        threshold_data = DeviceThresholdCreate(
            device_id=test_device.id,
    ...
    def test_detect_low_blood_oxygen(self, db_session, test_user, test_device, device_service):
        """测试检测血氧过低"""
        # 创建阈值配置
        threshold_data = DeviceThresholdCreate(
            device_id=test_device.id,

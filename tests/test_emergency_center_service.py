...
def test_call_120_success(db, emergency_center_service, test_user, test_emergency_center):
    """测试成功拨打120"""
    ...
    assert response.call_status in [EmergencyCallStatus.CONNECTED.value, EmergencyCallStatus.DIALING.value]
    assert response.dialed_at is not None
    assert response.location_sent is True
    assert response.health_summary_sent is True


def test_call_120_without_health_summary(db, emergency_center_service, test_user, test_emergency_center):
    """测试拨打120不发送健康档案"""
    request = Call120Request(
        user_id=test_user.user_id,
        sos_request_id=None,
...
def test_call_120_invalid_location(db, emergency_center_service, test_user, test_emergency_center):
    """测试拨打120位置格式错误"""
    request = Call120Request(
        user_id=test_user.user_id,
        sos_request_id=None,
...
def test_call_120_with_sos_request(db, emergency_center_service, test_user, test_emergency_center, test_sos_request):
    """测试拨打120关联SOS请求"""
    request = Call120Request(
        user_id=test_user.user_id,
...
def test_generate_health_summary(db, emergency_center_service, test_user, test_health_record, test_emergency_contact, test_device_data):
    """测试生成健康档案摘要"""
...
def test_generate_health_summary_with_anomalies(db, emergency_center_service, test_user, test_health_record, test_emergency_contact, test_device_data, test_anomaly):
    """测试生成健康档案摘要包含异常记录"""
    summary = emergency_center_service.generate_health_summary(db, test_user.user_id)
    
    assert len(summary.recent_anomalies) >= 1
    assert summary.recent_anomalies[0]['anomaly_type'] == test_anomaly.anomaly_type.value
...
def test_generate_health_summary_no_health_record(db, emergency_center_service, test_user, test_emergency_contact, test_device_data):
    """测试没有健康档案时生成摘要"""
    summary = emergency_center_service.generate_health_summary(db, test_user.user_id)
...
def test_dispatch_ambulance(db, emergency_center_service, test_user, test_emergency_center):
    """测试派出救护车"""
    # 先创建急救呼叫
    request = Call120Request(
        user_id=test_user.user_id,
...
def test_update_ambulance_location(db, emergency_center_service, test_user, test_emergency_center):
    """测试更新救护车位置"""
    # 先创建急救呼叫和救护车
    request = Call120Request(
        user_id=test_user.user_id,
    ...
    updated_ambulance = emergency_center_service.update_ambulance_location(db, location_data)
...
def test_track_ambulance(db, emergency_center_service, test_user, test_emergency_center):
    """测试追踪救护车"""
    # 先创建急救呼叫和救护车
    request = Call120Request(
        user_id=test_user.user_id,
...
def test_create_rescue_record(db, emergency_center_service, test_user, test_sos_request):
    """测试创建救援记录"""
...
def test_update_rescue_record(db, emergency_center_service, test_user, test_sos_request):
    """测试更新救援记录"""
    # 先创建救援记录
    incident_time = datetime.utcnow()
    alarm_time = datetime.utcnow()
    ...
    updated_record = emergency_center_service.update_rescue_record(db, record.id, update_data)
...
def test_get_emergency_centers_include_inactive(db, emergency_center_service, test_emergency_center):
    """测试获取所有急救中心(包含未启用的)"""
    # 创建一个未启用的急救中心
    from app.schemas.emergency_center import EmergencyCenterCreate
    ...
    # 只获取启用的
    active_centers = emergency_center_service.get_emergency_centers(db, active_only=True)
...
def test_full_emergency_call_flow(db, emergency_center_service, test_user, test_emergency_center, test_health_record, test_emergency_contact, test_device_data):
    """测试完整的急救呼叫流程"""
    # 1. 拨打120
    request = Call120Request(
        user_id=test_user.user_id,

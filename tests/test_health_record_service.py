...
from app.services.health_record_service import HealthRecordService, EncryptionService
...
class TestHealthRecordService:
    ...
    def test_create_health_record_invalid_blood_type(self, db, test_user, health_service):
        """测试创建健康档案时血型验证"""
        from pydantic import ValidationError
    ...
    def test_update_health_record(self, db, test_user, health_service):
        ...
        health_record = health_service.update_health_record(db, test_user.user_id, update_data)
    ...
    def test_add_medical_history(self, db, test_user, health_service):
        ...
        health_record = health_service.create_health_record(db, record_data, encrypt=False)
        ...
        medical_history = health_service.add_medical_history(db, health_record.id, history_data)
    ...
    def test_update_medical_history(self, db, test_user, health_service):
        ...
        updated_history = health_service.update_medical_history(db, medical_history.id, update_data)
    ...
    def test_add_medication(self, db, test_user, health_service):
        ...
        medication = health_service.add_medication(db, health_record.id, medication_data)
    ...
    def test_get_medications_current_only(self, db, test_user, health_service):
        ...
        # 获取当前用药
        current_medications = health_service.get_medications(db, health_record.id, current_only=True)
    ...
    def test_update_medication(self, db, test_user, health_service):
        ...
        updated_medication = health_service.update_medication(db, medication.id, update_data)

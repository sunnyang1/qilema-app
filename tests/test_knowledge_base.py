...
class TestKnowledgeCategoryService:
    ...
    def test_get_active_categories(self, db: Session):
        ...
        KnowledgeCategoryService.create_category(db, {"name": "分类2", "is_active": False})
    ...
    def test_get_category_tree(self, db: Session):
        ...
        # 创建子分类
        KnowledgeCategoryService.create_category(db, {"name": "基础常识", "parent_id": parent.id})
        KnowledgeCategoryService.create_category(db, {"name": "基本操作", "parent_id": parent.id})
    ...
    def test_update_category(self, db: Session):
        ...
        updated = KnowledgeCategoryService.update_category(db, category.id, {"name": "新名称"})
...
class TestKnowledgeArticleService:
    ...
    def test_get_published_articles(self, db: Session):
        """测试获取已发布文章列表"""
        # 创建文章
        KnowledgeArticleService.create_article(db, {
            "title": "文章1",
            "content": "内容1",
            "status": "published"
        })
    ...
    def test_get_article_detail(self, db: Session):
        """测试获取文章详情（增加浏览次数）"""
        article = KnowledgeArticleService.create_article(db, {
            "title": "测试文章",
            "content": "测试内容",
            "status": "published"
        })
    ...
    def test_update_article(self, db: Session):
        ...
        updated = KnowledgeArticleService.update_article(db, article.id, {
            "title": "新标题",
            "status": "published"
        })
    ...
    def test_get_related_articles(self, db: Session):
        ...
        article1 = KnowledgeArticleService.create_article(db, {
            "title": "文章1",
            "content": "内容1",
            "category_id": category.id,
            "status": "published"
